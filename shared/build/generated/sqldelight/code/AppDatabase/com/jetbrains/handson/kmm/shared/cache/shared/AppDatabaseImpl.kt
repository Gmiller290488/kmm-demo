...
import com.jetbrains.handson.kmm.shared.cache.AppDatabaseQueries
import com.jetbrains.handson.kmm.shared.cache.Rocket
import com.jetbrains.handson.kmm.shared.cache.SelectAllLaunchesInfo
...
private class AppDatabaseQueriesImpl(
  ...
  override fun <T : Any> selectAllLaunchesInfo(mapper: (
    ...
    name: String?,
    type: String?
  ) -> T): Query<T> = Query(1002595480, selectAllLaunchesInfo, driver, "AppDatabase.sq",
  ...
  override fun insertLaunch(
    ...
    notifyQueries(-349025720, {database.appDatabaseQueries.selectAllLaunchesInfo})
  }
  ...
  override fun insertRocket(
    ...
    notifyQueries(-164860567, {database.appDatabaseQueries.selectRocketById +
        database.appDatabaseQueries.selectAllLaunchesInfo})
  }

  override fun removeAllLaunches() {
    driver.execute(-1255327678, """DELETE FROM Launch""", 0)
    notifyQueries(-1255327678, {database.appDatabaseQueries.selectAllLaunchesInfo})
  }

  override fun removeAllRockets() {
    driver.execute(-843099298, """DELETE FROM Rocket""", 0)
    notifyQueries(-843099298, {database.appDatabaseQueries.selectRocketById +
        database.appDatabaseQueries.selectAllLaunchesInfo})
  }
  ...
  private inner class SelectRocketByIdQuery<out T : Any>(
    ...
    override fun execute(): SqlCursor = driver.executeQuery(211295070, """
    |SELECT * FROM Rocket
    |WHERE id = ?
    """.trimMargin(), 1) {
      bindString(1, id)
